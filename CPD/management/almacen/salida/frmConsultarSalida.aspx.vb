...
Public Class frmConsultarSalida : Inherits nspPaginaBase.PaginaBase
    ...
    Private Sub txbFechaFinal_TextChanged(sender As Object, e As EventArgs) Handles txbFechaFinal.TextChanged
        OP_SacaAreas()
    End Sub

    Protected Sub cmbArea_SelectedIndexChanged1(sender As Object, e As EventArgs)
        OP_SacaFolios()
    End Sub
    ...
    Public Sub OP_Listado()
        ...
        If (cmbFolio.SelectedItem.Value <> "0") Then
            ...
            If (Not IsNothing(listadoSalidaAlmacen)) Then
                detalleSalida = New Proceso_ObtenerDetallesSalidaAlmacen With {.tipoConsulta = tipoConsultaDetalleSalidaAlmacen.idSalida, .idSalida = listadoSalidaAlmacen.id}.Ejecutar().OrderBy(Function(l) l._codigoBarras).ToList
            End If
        End If
        lsvListado.DataSource = detalleSalida
        lsvListado.DataBind()
        controladorVistas("vst_folios")
    ...
    Protected Function controladorVistas(vst As String)
        Select Case vst
            ...
            Case "vst_folios"
                lista.Visible = True
                lista.Attributes.Remove("class")
                lista.Attributes("class") = "card animated wooble"
        End Select
        Return vst
    End Function
