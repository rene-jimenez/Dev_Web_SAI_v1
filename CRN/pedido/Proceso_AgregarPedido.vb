...
Namespace nspPedido
    Public Class Proceso_AgregarPedido : Inherits Accion(Of respuestaDelProceso)
        ...
        Public Sub New()
            MyBase.New("Proceso_AgregarPedido", "Agrega un registro a la base de datos")
        End Sub
        Protected Overrides Function OnEjecutar() As respuestaDelProceso
            ...
            Try
                Using scope As New TransactionScope()
                    ...
                    If resultadoValidación.respuesta = tipoRespuestaDelProceso.Completado Then
                        ...
                        Dim respuestaOficio = New CRN.nspOficio.Proceso_EditarOficio_EspecialPedidoSolicitud() With {.idOficio = oficio.id, .idArea = New CRN.nspOficio.Proceso_ObtenerUnOficio() With {.id = oficio.id}.Ejecutar.idArea, .idCargoPresupuestal = oficio.idCargoPresupuestal, .idEstatusOficio = oficio.idEstatusOficio,
            ...
            Catch ex As SqlException
                If ex.Number = 2601 Then
                    respuesta.comentario = "Ya existe un pedido para este oficio con el mismo proveedor."
